...
int main(int argc, char **argv){
    ...
    int measurementsNumber = nodeHandle.param<int>("meas_number", 5);
    if (measurementsNumber <= 0) measurementsNumber = 1;
    ...
    ros::Publisher batteryStatePublisher = nodeHandle.advertise<sensor_msgs::BatteryState>(ros::this_node::getName() + "/battery_state", 1);
    ros::ServiceServer displayServiceServer = nodeHandle.advertiseService(ros::this_node::getName() + "/display", displayCallback);

    ros::Rate rate(pubRate);
    while(ros::ok()){

        double cellVoltage1 = 0;
        double cellVoltage2 = 0;
        double powerVoltage = 0;

        for(int i = 0; i < measurementsNumber; i++){
            cellVoltage1 += youbotDashboard.getVoltage(youbot::VoltageSource::battery1);
            cellVoltage2 += youbotDashboard.getVoltage(youbot::VoltageSource::battery2);
            powerVoltage += youbotDashboard.getVoltage(youbot::VoltageSource::powersupply);
        }
        cellVoltage1 /= measurementsNumber;
        cellVoltage2 /= measurementsNumber;
        powerVoltage /= measurementsNumber;

        batteryState.header.stamp = ros::Time::now();
        batteryState.cell_voltage[0] = cellVoltage1;
        batteryState.cell_voltage[1] = cellVoltage2;
        batteryState.charge = powerVoltage;
